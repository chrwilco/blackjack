...
private int nextCardIndex;
	...
	public Deck(){
		...
		try{
		for (int i = 1; i <= 13; i++) {
			deck[count++] = new Card2('H', i);
		}
		for (int i = 1; i <= 13; i++) {
			deck[count++] = new Card2('S', i);
		}
		for (int i = 1; i <= 13; i++) {
			deck[count++] = new Card2('C', i);
		}
		for (int i = 1; i <= 13; i++) {
			deck[count++] = new Card2('D', i);
		}	
		}
	...
	public Card2 getCard(int index) throws InvalidDeckPositionException{
		isIndexGood(index);
		return deck[index];
	}
